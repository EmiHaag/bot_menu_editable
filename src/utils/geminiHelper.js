...
async function askGemini(userMessage) {
  ...
  let baseDelay = 600;   // Tiempo inicial de espera en milisegundos (0.6 segundos)
  for (let intento = 1; intento <= MAX_RETRIES; intento++) {
    try {
      ...
      // Si es un 503 (saturación) y todavía nos quedan intentos disponibles...
      if (isServiceUnavailable && intento < MAX_RETRIES) {
        // Calculamos un tiempo de espera que aumenta en cada intento (Backoff exponencial)
        const tiempoEspera = baseDelay * intento; 
        console.warn(`[GeminiHelper] Servidor saturado (503). Reintento ${intento}/${MAX_RETRIES} en ${tiempoEspera}ms...`);
        
        // Bloqueamos la ejecución esos milisegundos antes de volver a empezar el bucle
